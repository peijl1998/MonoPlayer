...
namespace libdash
{
    namespace framework
    {
        namespace input
        {
            ...
            {
                public:
                    DASHManager             (uint32_t maxCapacity, IDASHManagerObserver *multimediaStream, dash::mpd::IMPD *mpd, DASHMetrics * metrics);
                    virtual ~DASHManager    ();
                    ...
                    void        OnSegmentDownloaded     (float bytes, float duration);
