...
DASHManager::DASHManager        (uint32_t maxCapacity, IDASHManagerObserver* stream, IMPD* mpd, DASHMetrics* metrics) :
             readSegmentCount   (0),
             receiver           (NULL),
...
void DASHManager::UpdateMetrics() {
    this->metrics->SetBufferLevel(this->buffer->BufferLevel());
}

uint64_t DASHManager::BufferLevel ()
{
    return this->buffer->BufferLevel();
}
...
void        DASHManager::OnSegmentDownloaded    (float bytes, float duration)
{
    this->readSegmentCount++;
    this->metrics->AddThroughput(bytes, duration);
    // notify observers
}
