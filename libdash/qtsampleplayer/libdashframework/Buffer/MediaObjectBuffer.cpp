...
bool MediaObjectBuffer::Full() {
    return this->BufferLevel() >= this->maxcapacity;
}
float MediaObjectBuffer::BufferLevel ()
{
    float level = 0;
...
void            MediaObjectBuffer::Notify           ()
{
    for(size_t i = 0; i < this->observer.size(); i++)
        ...
        this->observer.at(i)->OnBufferStateChanged((int)((double)this->BufferLevel()/(double)this->maxcapacity*100.0));
}
